...
{capture name="title"}
    {if $block.wrapper === "blocks/wrappers/onclick_dropdown.tpl"}
        <a class="ty-account-info__title ty-account-info__title--dropdown" href="{"profiles.update"|fn_url}">
            {include_ext file="common/icon.tpl" class="ty-icon-user ty-account-info__title-icon"}&nbsp;<span class="ty-account-info__title-txt" {live_edit name="block:name:{$block.block_id}"}>{$title}</span>
        ...
        {hook name="profiles:my_account_menu"}
            {if $auth.user_id}
                {if $user_info.firstname || $user_info.lastname}
                    <li class="ty-account-info__item  ty-account-info__name {$dropdown_box_item_class}">{$user_info.firstname} {$user_info.lastname}</li>
                {else}
                    <li class="ty-account-info__item {$dropdown_box_item_class} ty-account-info__name">{$user_info.email}</li>
                {/if}
                <li class="ty-account-info__item {$dropdown_box_item_class}"><a class="ty-account-info__a underlined" href="{"profiles.update"|fn_url}" rel="nofollow" >{__("profile_details")}</a></li>
                {if $settings.General.enable_edp == "Y"}
                <li class="ty-account-info__item {$dropdown_box_item_class}"><a class="ty-account-info__a underlined" href="{"orders.downloads"|fn_url}" rel="nofollow">{__("downloads")}</a></li>
                {/if}
            {elseif $user_data.firstname || $user_data.lastname}
                <li class="ty-account-info__item  {$dropdown_box_item_class} ty-account-info__name">{$user_data.firstname} {$user_data.lastname}</li>
            {elseif $user_data.email}
                <li class="ty-account-info__item {$dropdown_box_item_class} ty-account-info__name">{$user_data.email}</li>
            {/if}
            <li class="ty-account-info__item {$dropdown_box_item_class}"><a class="ty-account-info__a underlined" href="{"orders.search"|fn_url}" rel="nofollow">{__("orders")}</a></li>
            {if $settings.General.enable_compare_products == 'Y'}
                {$compared_products_ids = $smarty.session.comparison_list}
                <li class="ty-account-info__item {$dropdown_box_item_class}"><a class="ty-account-info__a underlined" href="{"product_features.compare"|fn_url}" rel="nofollow">{__("view_comparison_list")}{if $compared_products_ids} ({$compared_products_ids|count}){/if}</a></li>
            {/if}
        {/hook}
    </ul>
