{if $location_data.page_id && $location_data.is_only_content === "YesNo::YES"|enum}
    <div class="tygh-content clearfix {if $layout_data.layout_width != "fixed"}container-fluid {else}container{/if} ty-cs-controller-{$runtime.controller} ty-cs-mode-{$runtime.mode} {if $runtime.action}ty-cs-action-{$runtime.action}{/if} {$container.user_class}" id="{$smarty.const.TILDA_PAGE_CONTAINER_ID}">
