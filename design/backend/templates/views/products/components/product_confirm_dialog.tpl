<div>
    <p>{__("predict.products.product_will_lose_some_features.warning", [$affected_feature_ids|count, "[product]" => $product, "[feature_search_url]" => "{fn_url("product_features.manage?feature_ids_search_key={$feature_ids_search_key}")}", "[category_search_url]" => "{fn_url("categories.manage?group_by_level=0&item_ids={","|implode:$not_affected_categories_ids}")}"])}</p>
</div>
