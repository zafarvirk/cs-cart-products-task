    ...
    {foreach $categories_tree as $category}
        ...
        {capture name="category_subtitle"}
            ...
            {if "MULTIVENDOR"|fn_allowed_for && $category.disabled}
                {$category.category}
            {else}
                <a class="row-status link--monochrome {if $category.status === "ObjectStatuses::NEW_OBJECT"|enum} manage-root-item-disabled{/if}{if !$category.subcategories} normal{/if}" href="{"addons.manage?category_id=`$category.category_id`"|fn_url}"{if !$category.subcategories} style="padding-{$direction}: 14px;"{/if} >{$category.category}</a>
            {/if}
        {/capture}
