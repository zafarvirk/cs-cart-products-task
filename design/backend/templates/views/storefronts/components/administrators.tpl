    ...
    <div class="table-responsive-wrapper longtap-selection">
        <table width="100%" class="table table-middle table--relative table-responsive">
            ...
            {foreach $storefront_admins as $admin}
                ...
                >
                    <td width="25%" class="row-status wrap" data-th="{__("person_name")}">{if $admin.firstname || $admin.lastname}<a href="{"profiles.update?user_id=`$admin.user_id`&user_type=`$admin.user_type`"|fn_url}" class="link--monochrome">{$admin.lastname} {$admin.firstname}</a>{else}-{/if}</td>
                    <td width="25%" data-th="{__("email")}"><a class="row-status link--monochrome" href="mailto:{$admin.email|escape:url}">{$admin.email}</a></td>
                </tr>
            {/foreach}
        </table>
    </div>
