<div class="select-block {if $purpose === "wysiwyg"}cm-select-bm-block{else}cm-add-block bm-action-existing-block{/if} {if $manage === "YesNo::YES"|enum}bm-manage{/if} {if $block.single_for_location}bm-block-single-for-location{/if}"
...
>
    ...
    <div class="select-block-description">
        ...
        <p><small class="muted">#{$block.block_id} </small>{$block_types[$block.type].description}</p>
    </div>
</div>
