...
if ($mode === 'details') {
    ...
    if (!empty($payment_method['processor_params']['is_stripe'])) {
        ...
        $total = $price_formatter->asCents(
            $order_info['total'] - $order_info['payment_surcharge'] + $payment_method['surcharge_value'],
            $payment_method['processor_params']['currency']
        );

        $view->assign('stripe_cart_total', $total);
        Tygh::$app['session']['stripe_cart_total'][$order_info['order_id']] = $order_info['total'] - $order_info['payment_surcharge'] + $payment_method['surcharge_value'];
        Tygh::$app['session']['stripe_cart_payment_surcharge'][$order_info['order_id']] = $payment_method['surcharge_value'];
    }
}
